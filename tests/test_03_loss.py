...
from popinn import eval_grid, ResidualTerm, eval_grid_flat_aux, Loss, FixedWeights
...
def test_residual_term_batch_size_routing(training_data, pde_residual_fn, pde_residual_term, p2inn):
    ...
    """
    res_term_eval_grid = ResidualTerm(
        name = 'pde',
        residual_fn = pde_residual_fn,
        eval_fn = eval_grid,
        batch_size = 2
    ...
    res_term_eval_grid_flat_aux = ResidualTerm(
        name = 'pde',
        residual_fn = pde_residual_fn,
...
def test_fixed_weights_combine(fixed_weights):
    ...
    keys.
    """
    res = {'pde': jnp.array(1.), 'dummy': jnp.array(3.)}
    check = fixed_weights.combine(res)
    correct = 5.
    assert jnp.allclose(check, correct, rtol=RTOL, atol=ATOL)
    with pytest.raises((KeyError)):
        res = {'abc': jnp.array(1.), 'dummy': jnp.array(3.)}
        check = fixed_weights.combine(res)
...
def test_loss_initialization(pde_residual_term, dummy_residual_term, fixed_weights):
    ...
    with pytest.raises((ValueError)):
        Loss(
            [pde_residual_term, pde_residual_term, dummy_residual_term],
            fixed_weights
            )
        
    # weights = None defaults to FixedWeights
    assert Loss([pde_residual_term, dummy_residual_term]).weights == FixedWeights({'pde': 1., 'dummy': 1.})

    # the set of weight keys must equal the set residual term names
    with pytest.raises((ValueError)):
        Loss(
            [pde_residual_term, dummy_residual_term],
            FixedWeights({'abc': 1., 'dummy': 1.})
            )

    #order of residual terms and weight key names doesn't matter:
    loss1 = Loss(
            [pde_residual_term, dummy_residual_term],
            FixedWeights({'dummy': 3., 'pde': 5.})
            )
    loss2 = Loss(
            [pde_residual_term, dummy_residual_term],
            FixedWeights({'pde': 5., 'dummy': 3.})
            )
...
def test_loss_model_differentiable(loss, training_data, p2inn):
    ...
    (loss_val, loss_dict), grads = eqx.filter_value_and_grad(
            lambda m: loss(m, training_data), has_aux=True)(p2inn)
